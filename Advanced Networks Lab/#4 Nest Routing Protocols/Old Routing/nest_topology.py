...
from nest.routing.routing_helper import *
...
#* Creating Left Nodes
for i in range(3):
    nodes_left.append(Node("node_left_" + str(i + 1)))

#* Creating Right Nodes
for i in range(2):
    nodes_right.append(Node("node_right_" + str(i + 1)))
...
# exp = Experiment("experiment_one")
# flow = Flow(nodes_left[0], nodes_right[1], right_interfaces[1][0].address, 0, 10, 5)

# exp.add_udp_flow(flow, target_bandwidth='10mbit')
