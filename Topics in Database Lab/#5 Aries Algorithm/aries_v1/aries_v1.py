    ...
    for i in range(instrus_count):
        ...
        if (ins.find('(') != -1):
            ...
            if (operation == 'ABORT'):
                transactionId = ins[ins.find('(') + 1:ins.find(')')]
                prevLSN = ds.active_transaction_table[transactionId].lastLSN
                # ds.active_transaction_table[transactionId].status = 'abort'
                ds.active_transaction_table[transactionId].lastLSN = LSN
                log_record = ds.LogRecord(LSN, prevLSN, transactionId, 'ABORT', '-', '-', '-', '-')
                ds.LOG_MAIN[LSN] = log_record

                LSN += 1

                #* As abort traverse backwards using prevLSN to create CLR's
                prev = prevLSN
                while prev != None:
                    record = ds.LOG_MAIN[prev]
                    if record.type == 'UPDATE':
                        prevLSN = ds.active_transaction_table[transactionId].lastLSN
                        ds.active_transaction_table[transactionId].lastLSN = LSN
                        clr = ds.LogRecord(LSN, prevLSN, transactionId, 'CLR-'+str(prev), record.dataItem, record.after, record.before, record.prevLSN)
                        ds.LOG_MAIN[LSN] = clr
                        LSN += 1
                    prev = record.prevLSN
